Spree::Order.class_eval do
  ...
  def update_adjustment_tax
    ...
    (adjustments.eligible - adjustments.tax - adjustments.shipping).each do |adjustment|
      ...
      default_tax_rate = Spree::TaxRate.first #Spree::TaxRate.find(1)
      default_tax_rate_amount = default_tax_rate.amount || 0
      
      adjustment_amount = (adjustment.amount - (adjustment.amount / (1+default_tax_rate_amount)))
      target = adjustment
      ...
      adj = target.adjustments.create(:amount => adjustment_amount,
                                      :source => adjustment,
